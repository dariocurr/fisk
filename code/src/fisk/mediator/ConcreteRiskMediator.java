...
import fisk.card.GoalCard.NumberOfTerritoriesGoalCard;
...
import fisk.player.ConcreteAIPlayer;
import fisk.player.ConcretePlayer;
import fisk.player.Player;
import fisk.player.RiskColor;
import fisk.stage.AttackStage;
import fisk.stage.MovingStage;
import fisk.stage.PreparationStage;
import fisk.stage.ReinforcementStage;
import fisk.stage.Stage;
import fisk.strategy.RiskStrategy;
import fisk.tank.Tank;
...
public class ConcreteRiskMediator extends RiskMediator {
    ...
    public void prepareGame(String humanPlayerName, RiskColor humanPlayerColor,
        ...
        this.facade.createRiskInterface(this.humanPlayer.getName(), this.humanPlayer.getColor().getColor(), playersData,
                stagesStrings);
    ...
    protected void updateAllContinents() {
        this.game.getContinents().forEach((continent) -> {
            this.players.stream().filter((player) -> (player.getTerritories().containsAll(continent.getTerritories())))
    ...
    public void startPreparationStage() {
        ...
        this.facade.showMessage(
                "The preparation stage begins.\n" + "If you want, look your goal in your panel (on the left).\n\n"
                        + "To start you must place " + this.humanPlayer.getFreeTanks().size() + " tank(s).\n"
                        + "To position the tanks, please click on the " + this.humanPlayer.getColor()
    ...
    public void nextPlayerPreparationStage() {
        if (this.isPreparationStageEnded()) {
            ...
            this.facade.updatePlayerData(this.humanPlayer.getTerritories().size(), this.currentPlayer.getName(),
                    this.currentStage.toString());
            this.notifyObservers(
                    "It's the turn of " + this.currentPlayer.getName() + " (" + this.currentPlayer.getColor() + ")");
    ...
    protected boolean isPreparationStageEnded() {
        return this.players.stream().noneMatch((p) -> (p.getFreeTanks().size() > 0));
    }
    ...
    public void nextPlayer() {
        if (!this.isEnded) {
            ...
            if (this.currentPlayer instanceof AIPlayer) {
                ...
                String text = "It's your turn. Please place " + this.humanPlayer.getFreeTanks().size() + " tank(s).";
                if (this.tutorial) {
                    text += "\nTo position the tanks, please click on the " + this.humanPlayer.getColor()
                            + " illuminated buttons.";
                }
    ...
    protected void releaseTanks() {
        ...
        numberOfTanks = this.currentPlayer.getContinents().stream()
                .map((continent) -> this.game.getContinentBonus(continent)).reduce(numberOfTanks, Integer::sum);
        for (int i = 0; i < numberOfTanks; i++) {
    ...
    protected void pause(Integer seconds) {
        /*
         * Long startTime = System.currentTimeMillis(); Long currentTime =
         * System.currentTimeMillis(); while (currentTime - startTime < seconds * 1000)
    ...
    public void exchangeTris(Tris tris) {
        if (this.checkTris(tris)) {
            ...
            /*
             * for (SymbolCard symbolCard : tris.getCards()) { if (symbolCard instanceof
             * TerritoryCard) { TerritoryCard territoryCard = (TerritoryCard) symbolCard; if
             * (this.currentPlayer.getTerritories().contains(territoryCard.getTerritory()))
             * { bonus += 2; } } }
             */
            bonus = tris.getCards().stream().filter((symbolCard) -> (symbolCard instanceof TerritoryCard))
                    .map((symbolCard) -> (TerritoryCard) symbolCard).filter((territoryCard) -> (this.currentPlayer
                            .getTerritories().contains(territoryCard.getTerritory())))
                    .map((_item) -> 2).reduce(bonus, Integer::sum);
            this.notifyObservers(
                    this.currentPlayer.getName() + " change " + tris + " and obtain " + bonus + " more tanks");
            for (int i = 0; i < bonus; i++) {
                this.currentPlayer.getFreeTanks()
                        .add(this.game.getTanksPool(this.currentPlayer.getColor()).releaseTank());
            }
            if (this.currentPlayer.equals(this.humanPlayer)) {
                this.facade.updatePlayerData(this.humanPlayer.getTerritories().size(), this.currentPlayer.getName(),
                        this.currentStage.toString());
    ...
    public boolean checkTris(Tris tris) {
        return this.game.getAllTrisBonus().keySet().stream().anyMatch((validTris) -> (validTris.equals(tris)));
    }
    ...
    public void moveTanks(Territory from, Territory to, Integer numberOfTanksToMove) {
        if (from.getOwnerPlayer().equals(to.getOwnerPlayer())) {
            ...
            this.facade.updateLabelsTerritories(involvedTerritories);
            this.notifyObservers(this.currentPlayer + " moved " + numberOfTanksToMove + " from " + from.getName()
                    + " to " + to.getName());
        }
    }
    ...
    public void handleAttack(Territory from, Territory to) {
        ...
        this.notifyObservers(this.currentPlayer.getName() + " attack " + attackedPlayer.toString() + " from "
        ...
        involvedTerritories.add(to);
        this.facade.updateLabelsTerritories(involvedTerritories);
        this.notifyObservers("Result of the clash: " + this.currentPlayer.getName() + " lost " + result[0]
                + " tank(s), " + attackedPlayer.toString() + " " + result[1]);
    }
    ...
    public void changeOwnerTerritory(Territory territory) {
        ...
        if (!this.checkEnd()) {
            if (attackedPlayer.getTerritories().isEmpty()) {
                ...
                this.notifyObservers("The player " + attackedPlayer.toString() + " has been killed by "
