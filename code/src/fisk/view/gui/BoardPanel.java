...
public class BoardPanel extends JPanel {
    ...
    public BoardPanel(RiskFacade facade, int width, int height, List<Territory> territories,
            Map<Continent, Integer> continentsBonus) {
        super();
        this.width = width;
        this.height = height;
        ...
        for (TerritoryButton territoryButton : this.territoriesButtons) {
            territoryButton.setBounds(territoryButton.getPositionX(), territoryButton.getPositionY(),
                    territoryButton.getWidth(), territoryButton.getHeight());
    ...
    protected void initTerritoryButtons() {
        try {
            ...
            while (in.hasNextLine()) {
                ...
                TerritoryButton t = new TerritoryButton(this.fromStringToTerritory(splitted_line[0].trim()),
                        Integer.parseInt(splitted_line[1].trim()), Integer.parseInt(splitted_line[2].trim()),
                        Integer.parseInt(splitted_line[3].trim()), Integer.parseInt(splitted_line[4].trim()));
    ...
    protected void drawLinesBetweenTerritories(Graphics g) {
        try {
            ...
            while (in.hasNextLine()) {
                ...
                Integer x1, y1, x2, y2;
                TerritoryButton firstTerritoryButton = this
                        .fromTerritoryToTerritoryButton(this.fromStringToTerritory(splitted_line[0]));
                ...
                if ((splitted_line[1].equalsIgnoreCase("LEFT")) || (splitted_line[1].equalsIgnoreCase("RIGHT"))) {
                    ...
                    TerritoryButton secondTerritoryButton = this
                            .fromTerritoryToTerritoryButton(this.fromStringToTerritory(splitted_line[1]));
    ...
    public void updateLabels(List<Territory> territories) {
        this.territoriesButtons.stream()
    ...
    public void setClickableTerritories(List<Territory> territories) {
        this.territoriesButtons.forEach((territoryButton) -> {
            if (territories.contains(territoryButton.getTerritory())) {
                ...
                territoryButton
                        .setBackground(territoryButton.getTerritory().getOwnerPlayer().getColor().getColor().darker());
            }
        });
    }
