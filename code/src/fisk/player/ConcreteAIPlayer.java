...
public class ConcreteAIPlayer extends AIPlayer {
    ...
    public Tris exchangeTris() {
        if (this.cards.size() < 3) {
            ...
            /*
             * for(Tris possibleTris: allCombinationsOfCards) { for(Tris validTris:
             * AIPlayer.trisBonus.keySet()) { if(validTris.equals(possibleTris)) {
             * allPossibleTris.add(possibleTris); } } }
             */
            allCombinationsOfCards.forEach((possibleTris) -> {
                AIPlayer.trisBonus.keySet().stream().filter((validTris) -> (validTris.equals(possibleTris)))
            ...
            if (allPossibleTris.isEmpty()) {
                ...
                for (Tris tris : allPossibleTris) {
                     ...
                     * (this.territories.contains(territoryCard.getTerritory())) { bonus += 2; } } }
                     */
            ...
            for (int j = i + 1; j < this.cards.size(); j++) {
                for (int k = j + 1; k < this.cards.size(); k++) {
                    allCombinationsOfCards
                            .add(new ConcreteTris(this.cards.get(i), this.cards.get(j), this.cards.get(k)));
                }
            }
        }
